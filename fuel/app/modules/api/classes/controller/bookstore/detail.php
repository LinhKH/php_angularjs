...
class Controller_Bookstore_Detail extends Controller_Base_Rest
{
    ...
    public function getTkanMAndRequestDeleteUser($kanId) {
        $arrTKanM = DB::select_array(['t_kan_m.del_req_flg', 't_kan_m.del_req_user_id', 't_kan_m.id',
            't_kan_m.del_req_dt', DB::expr('m_employee.emp_nm AS emp_nm')])
            ->from('t_kan_m')
            ->join('m_employee','left')
            ->on('t_kan_m.del_req_user_id','=','m_employee.emp_id')
            ->and_on('m_employee.del_flg','=',DB::expr('0'))
            ->where('t_kan_m.del_flg', '=', 0)
            ->where('t_kan_m.id', '=', $kanId)
            ->execute()-> current();
        return $arrTKanM; 
    }
